...
class DisplayBase
{
    ...
    template<typename T>
    struct HasActiveScreen<T, std::void_t<std::enable_if_t<std::is_pointer_v<
        decltype(std::declval<T&>().ActiveScreen())>>>> : std::true_type {};

    template<typename T, typename = void>
    struct HasLoadScreen : std::false_type {};
    template<typename T>
    struct HasLoadScreen<T, std::void_t<std::enable_if_t<std::is_void_v<
        decltype(std::declval<T&>().LoadScreen(
            std::declval<lv_obj_t*>()))>>>> : std::true_type {};

    template<typename T, typename = void>
    struct HasSetRotation : std::false_type {};
    template<typename T>
    struct HasSetRotation<T, std::void_t<std::enable_if_t<std::is_void_v<
        decltype(std::declval<T&>().SetRotation(
            std::declval<lv_display_rotation_t>()))>>>> : std::true_type {};

    template<typename T, typename = void>
    struct HasWidth : std::false_type {};
    template<typename T>
    struct HasWidth<T, std::void_t<std::enable_if_t<std::is_convertible_v<
        decltype(std::declval<T&>().Width()), int>>>> : std::true_type {};

    template<typename T, typename = void>
    struct HasHeight : std::false_type {};
    template<typename T>
    struct HasHeight<T, std::void_t<std::enable_if_t<std::is_convertible_v<
        decltype(std::declval<T&>().Height()), int>>>> : std::true_type {};
   ...
   protected:
    ~DisplayBase() noexcept
    {
        ...
        static_assert(HasUnlock<Derived>::value,
            "DisplayBase<D>: D 须实现 void Unlock()");
        ...
        static_assert(HasWidth<Derived>::value,
            "DisplayBase<D>: D 须实现 int Width()");
        static_assert(HasHeight<Derived>::value,
            "DisplayBase<D>: D 须实现 int Height()");
    }
};
