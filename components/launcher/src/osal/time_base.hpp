...
class TimeBase
{
    ...
    struct HasDelayMs<T,
                      std::void_t<std::enable_if_t<std::is_void_v<
                          decltype(std::declval<T&>().DelayMs(std::declval<uint32_t>()))>>>>
        : std::true_type
    {
    };
    ...
    struct HasTickMs<T,
                     std::void_t<std::enable_if_t<
                         std::is_convertible_v<decltype(std::declval<T&>().TickMs()), uint32_t>>>>
        : std::true_type
    {
    };
   ...
   protected:
    ~TimeBase() noexcept
    {
        ...
        static_assert(HasTickMs<Derived>::value, "TimeBase<D>: D 须实现 uint32_t TickMs()");
    }
};
