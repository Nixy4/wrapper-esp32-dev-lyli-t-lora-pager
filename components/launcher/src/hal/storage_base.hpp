...
class StorageBase
{
    ...
    struct HasNvsGet<T,
                     std::void_t<std::enable_if_t<std::is_convertible_v<
                         decltype(std::declval<T&>().NvsGet(std::declval<const char*>(),
    ...
    struct HasNvsSet<T,
                     std::void_t<std::enable_if_t<std::is_convertible_v<
                         decltype(std::declval<T&>().NvsSet(std::declval<const char*>(),
                                                            std::declval<const char*>(),
    ...
    struct HasNvsDel<T,
                     std::void_t<std::enable_if_t<std::is_convertible_v<
                         decltype(std::declval<T&>().NvsDel(std::declval<const char*>(),
                                                            std::declval<const char*>())),
                         bool>>>> : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasNvsIterateKeys<
        T,
        std::void_t<std::enable_if_t<std::is_convertible_v<
            decltype(std::declval<T&>().NvsIterateKeys(std::declval<const char*>(),
    ...
    struct HasSdMount<T,
                      std::void_t<std::enable_if_t<
                          std::is_convertible_v<decltype(std::declval<T&>().SdMount()), bool>>>>
        : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasSdUnmount<T,
                        std::void_t<std::enable_if_t<
                            std::is_convertible_v<decltype(std::declval<T&>().SdUnmount()), bool>>>>
        : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasSdAvailable<
        T,
        std::void_t<std::enable_if_t<
            std::is_convertible_v<decltype(std::declval<T&>().SdAvailable()), bool>>>>
        : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasSdListFiles<T,
                          std::void_t<std::enable_if_t<std::is_same_v<
                              decltype(std::declval<T&>().SdListFiles(std::declval<const char*>(),
                                                                      std::declval<const char*>())),
    ...
    template <typename T>
    struct HasSdFileSize<T,
                         std::void_t<std::enable_if_t<std::is_convertible_v<
                             decltype(std::declval<T&>().SdFileSize(std::declval<const char*>(),
                                                                    std::declval<size_t&>())),
                             bool>>>> : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasSdOpenRead<
        T,
        std::void_t<std::enable_if_t<std::is_pointer_v<decltype(std::declval<T&>().SdOpenRead(
    ...
    struct HasSdClose<T,
                      std::void_t<std::enable_if_t<std::is_void_v<
                          decltype(std::declval<T&>().SdClose(std::declval<FILE*>()))>>>>
        : std::true_type
    {
    };
   ...
   protected:
    ~StorageBase() noexcept
    {
        static_assert(HasNvsGet<Derived>::value,
                      "StorageBase<D>: D 须实现 bool NvsGet(const char* ns, const char* key, "
                      "std::string& out)");
        static_assert(HasNvsSet<Derived>::value,
                      "StorageBase<D>: D 须实现 bool NvsSet(const char* ns, const char* key, const "
                      "std::string& val)");
        ...
        static_assert(HasNvsIterateKeys<Derived>::value,
                      "StorageBase<D>: D 须实现 bool NvsIterateKeys(const char* ns, "
                      "std::vector<std::string>& keys)");
        static_assert(HasSdMount<Derived>::value, "StorageBase<D>: D 须实现 bool SdMount()");
        static_assert(HasSdUnmount<Derived>::value, "StorageBase<D>: D 须实现 bool SdUnmount()");
        static_assert(HasSdAvailable<Derived>::value,
                      "StorageBase<D>: D 须实现 bool SdAvailable()");
        static_assert(HasSdListFiles<Derived>::value,
                      "StorageBase<D>: D 须实现 std::vector<std::string> SdListFiles(const char* "
                      "dir, const char* ext)");
        static_assert(
            HasSdFileSize<Derived>::value,
        ...
        static_assert(HasSdClose<Derived>::value, "StorageBase<D>: D 须实现 void SdClose(FILE* f)");
    }
};
