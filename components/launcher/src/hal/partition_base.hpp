...
class PartitionBase
{
    ...
    template <typename T>
    struct HasPlanInstall<
        T,
        std::void_t<std::enable_if_t<std::is_convertible_v<
            decltype(std::declval<T&>().PlanInstall(
                std::declval<const char*>(), std::declval<size_t>(), std::declval<InstallSlot&>())),
            bool>>>> : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasFlashWrite<T,
                         std::void_t<std::enable_if_t<std::is_convertible_v<
                             decltype(std::declval<T&>().FlashWrite(std::declval<const uint8_t*>(),
                                                                    std::declval<size_t>())),
                             bool>>>> : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasFlashEnd<T,
                       std::void_t<std::enable_if_t<
                           std::is_convertible_v<decltype(std::declval<T&>().FlashEnd()), bool>>>>
        : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasFlashAbort<
        T,
        std::void_t<std::enable_if_t<
            std::is_convertible_v<decltype(std::declval<T&>().FlashAbort()), bool>>>>
        : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasSetBootByLabel<
        T,
        std::void_t<std::enable_if_t<std::is_convertible_v<
            decltype(std::declval<T&>().SetBootByLabel(std::declval<const char*>())),
            bool>>>> : std::true_type
    {
    };
    ...
    template <typename T>
    struct HasGetBootLabel<T,
                           std::void_t<std::enable_if_t<std::is_convertible_v<
                               decltype(std::declval<T&>().GetBootLabel(std::declval<char*>(),
                                                                        std::declval<size_t>())),
                               bool>>>> : std::true_type
    {
    };
   ...
   protected:
    ~PartitionBase() noexcept
    {
        static_assert(HasPlanInstall<Derived>::value,
                      "PartitionBase<D>: D 须实现 bool PlanInstall(const char* label, size_t "
                      "image_size, InstallSlot& slot)");
        static_assert(HasFlashBegin<Derived>::value,
                      "PartitionBase<D>: D 须实现 bool FlashBegin(const InstallSlot& slot, size_t "
                      "image_size)");
        static_assert(
            HasFlashWrite<Derived>::value,
            "PartitionBase<D>: D 须实现 bool FlashWrite(const uint8_t* data, size_t len)");
        static_assert(HasFlashEnd<Derived>::value, "PartitionBase<D>: D 须实现 bool FlashEnd()");
